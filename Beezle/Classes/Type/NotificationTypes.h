//
//  NotificationTypes.h
//  Beezle
//
//  Created by Me on 15/12/2011.
//  Copyright (c) 2011 __MyCompanyName__. All rights reserved.
//

#define NOTIFICATION_PAUSE @"Pause"

#define GAME_NOTIFICATION_BEE_LOADED @"BeeLoaded"
#define GAME_NOTIFICATION_BEE_REVERTED @"BeeReverted"
#define GAME_NOTIFICATION_BEE_FIRED @"BeeFired"
#define GAME_NOTIFICATION_BEE_SAVED @"BeeSaved"
#define GAME_NOTIFICATION_ENTITY_DISPOSED @"EntityDisposed"
#define GAME_NOTIFICATION_ENTITY_FROZEN @"EntityFrozen"
#define GAME_NOTIFICATION_ENTITY_UNFROZEN @"EntityUnfrozen"
#define GAME_NOTIFICATION_ENTITY_HIT @"EntityHit"
